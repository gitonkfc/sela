 ...
 */
function poly_jetpack_setup() {

	// Add theme support for Infinite Scroll.
	add_theme_support( 'infinite-scroll', array(
		'container' 	 => 'main',
		'render'    	 => 'poly_infinite_scroll_render',
		'footer'    	 => 'content',
	) );
...
add_action( 'after_setup_theme', 'poly_jetpack_setup' );

/**
 * Return early if Site Logo is not available.
 */
function poly_the_site_logo() {
...
if ( function_exists( 'jetpack_is_mobile' ) && class_exists( 'Jetpack_User_Agent_Info' ) ) {
	function poly_has_footer_widgets() {
	...
	add_filter( 'infinite_scroll_has_footer_widgets', 'poly_has_footer_widgets' );
}

/**
 * Flush the Rewrite Rules for the testimonials CPT after the user has activated the theme.
 */
function poly_flush_rewrite_rules() {
	flush_rewrite_rules();
}
add_action( 'after_switch_theme', 'poly_flush_rewrite_rules' );
 ...
 */
function poly_infinite_scroll_render() {
	while( have_posts() ) {
		the_post();
